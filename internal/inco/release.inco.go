...
func Release(root string) {
	// @inco: root != "", -panic("Release: root must not be empty")

	ov := loadOverlay(root)
	// @inco: len(ov.Replace) > 0, -panic("Release: no overlay entries — run gen first")
...
func ReleaseClean(root string) {
	// @inco: root != "", -panic("ReleaseClean: root must not be empty")

	ov := loadOverlay(root)
	// @inco: len(ov.Replace) > 0, -panic("ReleaseClean: no overlay entries")
